...
export const ChitGroupEdit: React.FC = () => {
  ...
  const fetchChitDetails = async () => {
    try {
      ...
      if (data.status !== 'DRAFT') {
        toast.error('Only chit groups in DRAFT status can be edited');
        navigate(`/organizer/chit-groups/${id}`);
        return;
      }
      setAllocatedShares(data.allocated_shares);
      setFormData({
        chit_name: data.chit_name,
        description: data.description || '',
        total_shares: data.total_shares.toString(),
        duration_months: data.duration_months.toString(),
        monthly_installment_per_share: data.monthly_installment_per_share.toString(),
        total_chit_value: data.total_chit_value.toString(),
        maintenance_charge: data.maintenance_charge.toString(),
        maintenance_charge_type: data.maintenance_charge_type,
        start_date: data.start_date,
        installment_due_day: data.installment_due_day.toString(),
      });
    } catch (error) {
  ...
  const validate = (): boolean => {
    ...
    if (shares < allocatedShares) {
      toast.error(`Total shares cannot be less than currently allocated shares (${allocatedShares})`);
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/chit-groups/${id}`, payload);
      toast.success('Chit group updated successfully!');
      navigate(`/organizer/chit-groups/${id}`);
    } catch (error: any) {
      const detail = error.response?.data?.detail;
      toast.error(typeof detail === 'string' ? detail : 'Failed to update chit group');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto pb-12">
      <div className="flex items-center space-x-3 mb-6">
        <Link to={`/organizer/chit-groups/${id}`} className="p-2 hover:bg-slate-100 dark:hover:bg-gray-800 rounded-full transition-colors text-slate-600 dark:text-slate-400">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-xl font-bold text-slate-800 dark:text-white">Edit Chit Group</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                name="total_shares"
                required
                min={Math.max(2, allocatedShares)}
              ...
              {allocatedShares > 0 && (
                <p className="text-[10px] text-slate-400 mt-1">
                  Cannot be less than {allocatedShares} (currently allocated shares)
                </p>
              )}
            </div>
